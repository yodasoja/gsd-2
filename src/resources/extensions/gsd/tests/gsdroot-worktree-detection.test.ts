 ...
 * is inside a .gsd/worktrees/<name>/ structure so artifact projections stay
...
describe("gsdRoot() worktree detection (#2594)", () => {
  ...
  test("returns worktree-local .gsd when worktree .gsd does not exist yet", () => {
    const worktreeBase = join(projectGsd, "worktrees", "M008");
    ...
    assert.equal(
      result,
      join(worktreeBase, ".gsd"),
      `Expected worktree-local .gsd (${join(worktreeBase, ".gsd")}), got ${result}.`,
    );
  });
  ...
  test("returns worktree-local .gsd when basePath is a real git worktree inside .gsd/worktrees/", () => {
    ...
    assert.equal(
      gsdResult,
      join(worktreeBase, ".gsd"),
      `Expected worktree-local .gsd (${join(worktreeBase, ".gsd")}), got ${gsdResult}`,
    );
